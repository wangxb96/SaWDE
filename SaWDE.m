...
%% Problem 1
Problem = {'grammatical_facial_expression01.txt','SemeionHandwrittenDigit.txt','isolet5.txt','MultipleFeaturesDigit.txt',...
           'HAPTDataSet.txt','har.txt','UJIIndoorLoc.txt','MadelonValid.txt','OpticalRecognitionofHandwritten.txt',...
           'ConnectionistBenchData.txt','wdbc.txt','LungCancer.txt'};
%% Problem 2
...
totalTime = 1;
 while time <= totalTime     
   %% the values and indices of the best solutions
    ...
    genForChange = 20;
    MaxFESpre = 1000000; % Function Evaluation Times
...
%         weight1 = zeros(1,D);
      ...
      if (mixValMax == 1 && nfeatMax <= 0.5 * D ) || (mixValfitMin == 1 && nfeatfitMin <= 0.5 * D)
          break;
      end
    end
    time = time + 1;     
